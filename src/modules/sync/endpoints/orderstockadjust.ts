...
/**
 * @swagger
 * /sync/orderstockadjust/sincronize:
 *   post:
 *     summary: Sincroniza orderstockadjust do Sincronizador
 *     tags: [Sync]
 *     security:
 *       - ApiKeyAuth: []
 *     requestBody:
 ...
 */
router.post('/orderstockadjust/sincronize', async (req: Request, res: Response) => {
  const conn = await pool.getConnection()
  try {
    await conn.beginTransaction()
    await conn.query(`USE \`${req.syncClient!.schemaName}\``)

    const { Pedido, AjusteEstoque, Items, Totalizador, tb_institution_id } = req.body
    ...
    const [exAdj] = await conn.query<any[]>(
      `SELECT id FROM tb_order_stock_adjust WHERE id=? AND tb_institution_id=? AND terminal=? LIMIT 1`,
      [orderId, institutionId, AjusteEstoque?.terminal]
    )
    if (exAdj.length === 0) {
      await conn.query(
        `INSERT INTO tb_order_stock_adjust (id, tb_institution_id, terminal, created_at, updated_at)
         VALUES (?,?,?,NOW(),NOW())`,
        [orderId, institutionId, AjusteEstoque?.terminal]
      )
    }
    ...
    await conn.rollback()
    logger.error('Erro em /orderstockadjust/sincronize', { err, client: req.syncClient })

...
import {
  fetchVitrine, fetchResolvedConfigs, fetchResolvedConfigsByKey,
  saveConfigValue, saveConfigValueByKey,
...
export async function list(req: Request, res: Response): Promise<void> {
  try {
    const query = await parseListQuery(req, 'interface-configs')
...
export async function putValueByKey(req: Request, res: Response): Promise<void> {
  const moduleKey = String(req.params.moduleKey ?? '')
  if (!MODULE_KEY_RE.test(moduleKey)) {
    res.status(400).json({ error: 'moduleKey inválido' })
    return
  ...
  if (body === null) return
  try {
    await saveConfigValueByKey(req.institution!, moduleKey, name, body)
    logger.info('Valor de configuração salvo (por chave)', {
      institutionId: req.institution!.institutionId,
      moduleKey, name, target: body.target,
    })
    res.json({ ok: true })
  } catch (err) {
    handleError(res, err, 'interface-configs/key/:moduleKey/:name PUT')
  }
}
